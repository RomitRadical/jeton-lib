 ...
 * @param {PrivateKey} spenderPrivKey - the private key of the escrow beneficiary
 * @param {string} escrowMsg - the message for the outcome
 ...
 * @returns {Transaction}
 */
Transaction.prototype.signEscrow = function(
  inputIndex,
  spenderPrivKey,
  escrowMsg,
  oracleSig,
  ...
  let p2pkhSig = Sighash.sign(
    this,
    spenderPrivKey,
    sighash,
    inputIndex,
    subscript,
    BN.fromNumber(input.output.satoshis),
    Transaction.P2SHFlags
  );

  let spendingScript = Script.buildPublicKeyHashIn(
    spenderPrivKey.toPublicKey(),
    p2pkhSig,
    sighash
  );
...
Transaction.prototype.verifyScriptSig = function(inputIndex) {
  ...
  let verified = Interpreter().verify(
    inScript,
    outScript,
    this,
    inputIndex,
...
Transaction.constructSplitTx = function(
  inputUtxos,
  destinationAddr,
  amountToSend,
  minerFee
